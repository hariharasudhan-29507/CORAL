...
let supabase = null;
const profileColumns = "id, username, nickname, bio, avatar_url, account_visibility, email, phone, created_at, updated_at";
...
function mapFriend(row, profile) {
    return {
...
function mapFriendRequest(row, requesterProfile, addresseeProfile) {
    return {
        id: row.id,
...
function nicknameForUser(user) {
    const nickname = user.name || user.email || user.phone;
    if (!nickname)
        throw new Error("Authenticated user is missing profile identity.");
    return nickname;
}
...
export async function ensureProfile(user) {
    ...
    if (error)
        throw error;
    if (!data)
        throw new Error("Profile could not be loaded from Supabase.");
...
export async function updateProfile(user, input) {
    ...
    if (input.username !== undefined)
        patch.username = input.username ? normalizeUsername(input.username) : null;
    ...
    if (input.accountVisibility !== undefined)
        patch.account_visibility = input.accountVisibility;
...
export async function searchProfileByUsername(user, username) {
    const client = requireSupabase();
    await ensureProfile(user);
    ...
    if (error)
        throw error;
    if (!profileRow || profileRow.id === user.id)
        return { profile: null, relationship: "none" };
...
export async function listFriends(user) {
    ...
    return {
        friends: (friendRows ?? []).map((row) => mapFriend(row, profilesById.get(row.friend_id))),
    };
}
export async function listFriendRequests(user) {
    const client = requireSupabase();
    await ensureProfile(user);
    ...
    if (error)
        throw error;
    const profileIds = [
        ...new Set((requestRows ?? []).flatMap((row) => [row.requester_id, row.addressee_id]).filter((id) => id !== user.id)),
    ];
...
export async function requestFriendByUsername(user, username, message) {
    const client = requireSupabase();
    await ensureProfile(user);
    const normalized = normalizeUsername(username);
    ...
    if (targetError)
        throw targetError;
    if (!target)
        throw new Error("No Coral user was found for that username.");
    ...
    if (target.account_visibility === "public") {
        ...
        return { status: "accepted", profile: mapProfile(target) };
    }
    const { data, error } = await client
        .from("friend_requests")
        .upsert({
    ...
    if (error)
        throw error;
    return { status: "pending", request: mapFriendRequest(data, undefined, mapProfile(target)) };
}
export async function respondToFriendRequest(user, requestId, action) {
    const client = requireSupabase();
    await ensureProfile(user);
...
export async function removeFriend(user, friendId) {
    const client = requireSupabase();
    if (friendId === user.id)
        throw new Error("Choose another friend.");
...
export async function setFavoriteFriend(user, friendId, favorite) {
    const client = requireSupabase();
    const { data, error } = await client
        .from("friends")
...
export async function listConversationParticipantIds(conversationId) {
    const client = requireSupabase();
    const { data, error } = await client
        .from("conversation_participants")
        .select("user_id")
...
export async function listConversations(user) {
    ...
    const [{ data: conversationRows, error: conversationError }, { data: allParticipantRows, error: allParticipantError }] = await Promise.all([
        client
            .from("conversations")
    ...
    if (allParticipantError)
        throw allParticipantError;
    const userIds = [...new Set((allParticipantRows ?? []).map((row) => row.user_id))];
...
export async function createDmConversation(user, targetUserId) {
    const client = requireSupabase();
    if (targetUserId === user.id)
        throw new Error("Choose another person to message.");
    await ensureProfile(user);
    ...
    if (candidateIds.length > 0) {
        ...
        if (targetMatchesError)
            throw targetMatchesError;
        const matchingIds = (targetMatches ?? []).map((row) => row.conversation_id);
...
async function getConversationPeerId(conversationId, userId) {
    const client = requireSupabase();
    const { data, error } = await client
        .from("conversation_participants")
        .select("user_id")
...
function mapChatMessage(row) {
    if (!row.sender_id)
        throw new Error("Message row is missing sender_id.");
    if (!row.conversation_id)
        throw new Error("Message row is missing conversation_id.");
    let body = row.body ?? "";
    let mediaUrl = undefined;
    let mediaType = "text";
    let audioDuration = undefined;
    let audioWaveform = undefined;
